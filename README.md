...
├── style.css      estilos, tokens de cor e temas
├── script.js      catálogo, validação, cálculo, protocolo e consulta
├── README.md
└── img/           logo (clara e escura), favicon e ícones           logo, favicon e ícones das categorias
```
...
- As solicitações são guardadas no `localStorage` do navegador, sob as chaves
  `wupus:solicitacoes`, `wupus:contador` e `wupus:tema`. O botão
